...
#define OS_IPC_ERR_ACAPI          (TError)(0x1<<9)    /* 线程不能以阻塞方式访问IPC对象          */

/* IPC对象属性，内核代码使用 */
#define OS_IPC_PROP_DEFAULT       (TProperty)(0x0)
#define OS_IPC_PROP_READY         (TProperty)(0x1<<0) /* IPC对象已经被初始化                    */
#define OS_IPC_PROP_PREEMP_AUXIQ  (TProperty)(0x1<<1) /* 辅助线程阻塞队列采用优先级调度方案     */
#define OS_IPC_PROP_PREEMP_PRIMIQ (TProperty)(0x1<<2) /* 基本线程阻塞队列采用优先级调度方案     */
#define OS_IPC_PROP_AUXIQ_AVAIL   (TProperty)(0x1<<8) /* 辅助线程阻塞队列里存在被阻塞的线程     */
#define OS_IPC_PROP_PRIMQ_AVAIL   (TProperty)(0x1<<9) /* 基本线程阻塞队列里存在被阻塞的线程     */
...
#define OS_IPC_OPT_WAIT           (TOption)(0x1<<1)   /* 永久方式等待IPC                        */
#define OS_IPC_OPT_TIMEO          (TOption)(0x1<<2)   /* 时限方式等待标记                       */
#define OS_IPC_OPT_UARGENT        (TOption)(0x1<<3)   /* 消息队列、邮件使用                     */
#define OS_IPC_OPT_AND            (TOption)(0x1<<4)   /* 标记事件标记操作是AND类型              */
#define OS_IPC_OPT_OR             (TOption)(0x1<<5)   /* 标记事件标记操作是OR类型               */
#define OS_IPC_OPT_CONSUME        (TOption)(0x1<<6)   /* 事件标记使用                           */

#define OS_IPC_OPT_SEMAPHORE      (TOption)(0x1<<16)  /* 标记线程阻塞在信号量的线程阻塞队列中   */
#define OS_IPC_OPT_MUTEX          (TOption)(0x1<<17)  /* 标记线程阻塞在互斥量的线程阻塞队列中   */
#define OS_IPC_OPT_MAILBOX        (TOption)(0x1<<18)  /* 标记线程阻塞在邮箱的线程阻塞队列中     */
#define OS_IPC_OPT_MSGQUEUE       (TOption)(0x1<<19)  /* 标记线程阻塞在消息队列的线程阻塞队列中 */
#define OS_IPC_OPT_FLAGS          (TOption)(0x1<<20)  /* 标记线程阻塞在事件标记的线程阻塞队列中 */

#define OS_IPC_OPT_READ_DATA      (TOption)(0x1<<24)  /* 接收邮件或者消息                       */
#define OS_IPC_OPT_WRITE_DATA     (TOption)(0x1<<25)  /* 发送邮件或者消息                       */
