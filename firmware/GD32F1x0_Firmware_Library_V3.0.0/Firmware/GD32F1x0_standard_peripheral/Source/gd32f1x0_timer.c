...
void timer_init(uint32_t timer_periph, timer_parameter_struct* timer_initpara)
{
    ...
    if((TIMER5 != timer_periph)){
        ...
        TIMER_CTL0(timer_periph) |= (uint32_t)timer_initpara->timer_clockrivision;
    }
...
{
    if(timer_ccsel){
        TIMER_CTL1(timer_periph) |= (uint32_t)timer_ccsel;
    }else{
        TIMER_CTL1(timer_periph) &= (uint32_t)timer_ccsel;
    }
}
...
{
    if(timer_outsel){
        TIMER_CTL1(timer_periph) |= (uint32_t)timer_outsel;
    }else{
        TIMER_CTL1(timer_periph) &= (uint32_t)timer_outsel;
    }
}
